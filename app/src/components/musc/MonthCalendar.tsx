...
import { fetchMonthActivity, type MonthActivity, type DayActivity, type DayStatus } from '../../lib/monthActivity'
...
export default function MonthCalendar() {
  ...
  const next = () => {
    ...
    if (!isCurrent) setYm(({ y, m }) => (m === 11 ? { y: y + 1, m: 0 } : { y, m: m + 1 }))
  }
  const isCurrentMonth = ym.y === now.getFullYear() && ym.m === now.getMonth()

  // offset do 1º dia (semana começa na segunda)
            ...
            {data.days.map((d) => {
              ...
              return (
                <button key={d.date} onClick={() => setSel(selected ? null : d)}
                  className="aspect-square rounded-lg flex items-center justify-center text-[11px] font-bold transition active:scale-90"
                  style={{ background: st.bg, color: st.color, border: selected ? '2px solid #0F172A' : d.status === 'future' ? '1px dashed #E2E8F0' : 'none' }}>
            ...
            <div className="mt-3 p-3 rounded-xl text-[12px]" style={{ background: '#F8FAFC' }}>
              ...
              {sel.status === 'full' || sel.status === 'partial' ? (
                <div style={{ color: T.sub }} className="mt-0.5">
                  {sel.sessionName} · {sel.setsDone}{sel.setsPlanned ? `/${sel.setsPlanned}` : ''} séries · {sel.volume?.toLocaleString('pt-BR')}kg · {sel.durationMin}min
                  {sel.status === 'partial' && <span className="font-semibold" style={{ color: '#B45309' }}> · treino parcial</span>}
                </div>
              ) : (
